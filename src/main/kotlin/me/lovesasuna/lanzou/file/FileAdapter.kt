 ...
 */
class FileAdapter(private val user: User, private val file: File) : Multipart("test"), Uploadable {
    fun upload(): Boolean {
        return uploadFile(user, file, 0)
    }

    override fun upload(user: User, file: File, loc: Int): Boolean {
        return uploadFile(user, file, loc)
    }

    private fun uploadFile(user: User, file: File, loc: Int): Boolean {
